...
export function useProject({ profile, triggerSyncIndicator }: UseProjectProps) {
  ...
  const addProject = async (projectData: {
    ...
    if (!listOwner?.$isLoaded) {
      logger.error("Cannot create a new project instance: projectsList.$jazz.owner is not loaded.");
      return undefined;
    }
  ...
  const updateProject = async (
    ...
    if (projectData.title !== undefined && projectToUpdate.title !== projectData.title) {
      projectToUpdate.$jazz.set("title", projectData.title);
      changed = true;
    }

    if (projectData.year !== undefined && projectToUpdate.year !== projectData.year) {
