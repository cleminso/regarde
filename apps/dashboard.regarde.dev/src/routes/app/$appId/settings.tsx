...
function SettingsPage(): React.ReactElement {
  ...
  const shouldShowSecretWarning = 
    isProviderRequiringSecret === true && 
    isWebhookSecretMissing === true;
   ...
   * URL format: https://api.regarde.dev/webhooks/{provider}/{appId}
  ...
  const handleCopyUrl = async () => {
    ...
    const url = getWebhookUrl(selectedApp.paymentProvider, selectedApp.$jazz.id);
        ...
        <div className="mt-6 space-y-6">
          ...
          {shouldShowSecretWarning === true && (
            <div className="flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950">
              ...
              <div>
                <h3 className="font-semibold text-red-800 dark:text-red-200">Webhook Secret Required</h3>
                <p className="mt-1 text-sm text-red-700 dark:text-red-300">
                  Your {selectedApp.paymentProvider} webhook secret is not configured. Webhooks will
                  not be processed until you add it below.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                ...
                <div className="mt-1 flex gap-2">
                  <Input
                    value={getWebhookUrl(selectedApp.paymentProvider, selectedApp.$jazz.id)}
                    readOnly
              ...
              <div>
                  ...
                  (() => {
                    ...
                    return (
                      <div className="mt-1 flex gap-2">
                        <Input
                          ...
                          value={hasSecret === true ? selectedApp.webhookSecret : "Not configured"}
                          readOnly
                          className={`bg-muted ${hasSecret === false ? "text-gray-400 italic" : ""}`}
                        />
                        <Button variant="outline" onClick={() => setIsEditingSecret(true)}>
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        disabled={newSecret.trim().length === 0 || saveStatus === "saving"}
                      >
                        {saveStatus === "saving"
                          ? "Saving..."
                          : saveStatus === "saved"
                ...
                {selectedApp.paymentProvider === "lemonsqueezy" && (
                  <p className="mt-2 text-sm text-gray-600">
                    This secret was generated by Regarde. If you rotate it here, make sure to update
                    it in your LemonSqueezy dashboard too.
                  </p>
                )}
                {selectedApp.paymentProvider === "stripe" && (
                  <p className="mt-2 text-sm text-gray-600">
                    Find this in your Stripe Dashboard → Developers → Webhooks → [Your Endpoint] →
                ...
                {selectedApp.paymentProvider === "polar" && (
                  <p className="mt-2 text-sm text-gray-600">
                    Find this in your Polar Dashboard → Settings → Webhooks → [Your Endpoint].
                  </p>
                )}
              </div>
            </div>
