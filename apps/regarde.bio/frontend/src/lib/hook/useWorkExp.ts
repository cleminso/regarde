...
export function useWorkExp({ profile, triggerSyncIndicator }: UseWorkExpProps) {
  ...
  const updateWorkExp = async (
    ...
    if (workExpData.from !== undefined && workExpToUpdate.from !== workExpData.from) {
      workExpToUpdate.$jazz.set("from", workExpData.from);
      changed = true;
    }

    if (workExpData.title !== undefined && workExpToUpdate.title !== workExpData.title) {
      workExpToUpdate.$jazz.set("title", workExpData.title || "Work Experience");
      changed = true;
    }

    if (workExpData.company !== undefined && workExpToUpdate.company !== workExpData.company) {
  ...
  const deleteWorkExp = async (workExpId: string) => {
    ...
    if (workExpIndex !== -1) {
      ...
      logger.error(`Work experience with id ${workExpId} not found for deletion.`);
    }
  };
