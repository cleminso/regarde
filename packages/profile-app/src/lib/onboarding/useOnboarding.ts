...
export function useOnboarding() {
  ...
  const currentNickname = me?.profile?.onboarding?.nickname || '';
  const isNicknameActive = me?.profile?.onboarding?.isActive || false;
  ...
  useEffect(() => {
    ...
    if (registrationInProgress.current || hasProcessedPendingNickname.current || globalRegistrationInProgress) return;
  ...
  }, [isAuthenticated, me?.id, isAccountReady, currentNickname, navigate, getValidKey]);
  ...
  }, [isAuthenticated, me?.id, isAccountReady, currentNickname, clerk, navigate, isProcessing, getValidKey]);

  // Update nickname (for existing users)
  const update = useCallback(async (nickname: string) => {
    if (!nickname || nickname === currentNickname || isProcessing || registrationInProgress.current) return;
    if (!me?.profile?.onboarding) {
      setError('Onboarding data not available. Please refresh and try again.');
      return;
    }
  ...
  }, [me?.id, me?.profile?.onboarding, currentNickname, isProcessing, getValidKey]);
