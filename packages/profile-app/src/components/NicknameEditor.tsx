...
const NicknameEditor: React.FC<NicknameEditorProps> = ({ accountId, currentNickname }) => {
  const [nickname, setNickname] = useState(currentNickname || '');
  const [buttonState, setButtonState] = useState<'View' | 'Update' | 'N/A' | 'Checking...' | 'Error'>('View');
  ...
  useEffect(() => {
    ...
    if (currentNickname) {
      ...
      setButtonState('Checking...'); // Or some other initial state if accountId is present
       if (accountId && (currentNickname === undefined || currentNickname === '')) {
  ...
  const handleButtonClick = async () => {
    setError(null);
    if (!accountId) {
        navigate('/login'); // Or some other appropriate action
        return;
    }
    ...
    if (buttonState === 'View') {
      ...
      try {
        // Now using the imported API function
        const success = await updateUserNickname(accountId, nickname, currentNickname);
        if (success) {
  ...
  // Base button classes
  const baseButtonClasses = "px-4 py-2 text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:cursor-not-allowed";

  // State-specific button classes
  let buttonSpecificClasses = "";
  if (buttonState === 'N/A' || buttonState === 'Error') {
    buttonSpecificClasses = "bg-red-600 hover:bg-red-700 text-white focus:ring-red-500";
  } else if (buttonState === 'View') {
    buttonSpecificClasses = "bg-blue-600 hover:bg-blue-700 text-white focus:ring-blue-500";
  } else if (buttonState === 'Update') {
    buttonSpecificClasses = "bg-green-600 hover:bg-green-700 text-white focus:ring-green-500";
  ...
  const isDisabled = isLoading || buttonState === 'N/A' || buttonState === 'Checking...' || buttonState === 'Error' || !accountId;
  if (isDisabled && !(buttonState === 'Checking...' || isLoading || buttonState === 'N/A' || buttonState === 'Error')) {
  ...
  return (
    <div className="my-4 p-4 border border-gray-200 rounded-md dark:border-gray-700">
      <label htmlFor="nicknameInput" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
        Nickname
      </label>
