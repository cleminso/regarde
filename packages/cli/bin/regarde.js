...
program
    ...
    .requiredOption("-p, --providerName <provider>", "Payment provider (e.g., lemonsqueezy)")
    ...
    .action(async (options) => {
        ...
        if (providerName !== "lemonsqueezy" && providerName !== "stripe") {
            console.error(chalk.red("Error: providerName must be 'lemonsqueezy' or 'stripe'"));
            process.exit(1);
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                spinner.fail(`Registration failed: ${response.status} ${response.statusText}`);
            ...
            console.log(`  4. Select 'subscription_created', 'subscription_updated', etc. events.`);
