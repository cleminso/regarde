...
import { AppsRecord, RegistryWorkerAccount, RegistryAppMetadata } from "@regarde-dev/sdk/registry";
...
export const lemonSqueezyWebhookHandler = (
    ...
    return async (c: any) => {
        try {
            ...
            const loadedMetadata = await (appMetadata as any).$jazz.ensureLoaded({ app: true });
            if (!loadedMetadata || !loadedMetadata.app) {
                return c.json({ error: "App data unavailable" }, 500);
            }

            const app = await loadedMetadata.app.$jazz.ensureLoaded({ payments: true });
            ...
            if (!userAccountId) {
                console.warn("[Webhook] Warning: No user_id in custom_data. Using 'unverified_user'.");
