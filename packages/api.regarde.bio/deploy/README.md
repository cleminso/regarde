...
chmod +x packages/api.regarde.bio/deploy/scripts/bootstrap-vm.sh
./packages/api.regarde.bio/deploy/scripts/bootstrap-vm.sh

# 3. Configure environment
cp packages/api.regarde.bio/deploy/config/.env.template packages/api.regarde.bio/.env
nano packages/api.regarde.bio/.env  # Edit with your actual values

# 4. Deploy the application
./packages/api.regarde.bio/deploy/scripts/deploy-production.sh
```
...
All configuration is centralized in the `.env` file located in `packages/api.regarde.bio/.env`:

> **Note**: The .env file is located in the api.regarde.bio directory (not the project root) so that Bun can automatically load it at runtime using its native .env support.

```bash
# View current configuration
cat packages/api.regarde.bio/.env

# Edit configuration
nano packages/api.regarde.bio/.env

# Template with all options
cat packages/api.regarde.bio/deploy/config/.env.template
```
...
# List backups
./packages/api.regarde.bio/deploy/scripts/manage-backups.sh list

# Rollback to previous version
./packages/api.regarde.bio/deploy/scripts/rollback.sh
```

## 📁 Project Structure

```
packages/api.regarde.bio/deploy/
├── scripts/
...
- [ ] **Repository Setup**
  ...
  - [ ] Run bootstrap: `./packages/api.regarde.bio/deploy/scripts/bootstrap-vm.sh`

- [ ] **Configuration**
  - [ ] Copy template: `cp packages/api.regarde.bio/deploy/config/.env.template packages/api.regarde.bio/.env`
  - [ ] Edit configuration: `nano packages/api.regarde.bio/.env`
  - [ ] Set required values: `APP_PUBLIC_HOSTNAME`, `SSL_CERTIFICATE_EMAIL`, Jazz credentials

- [ ] **Deployment**
  - [ ] Run deployment: `./packages/api.regarde.bio/deploy/scripts/deploy-production.sh`
  - [ ] Verify API: `curl -s https://your-domain.com/health | jq .`

### Regular Updates

- [ ] Pull changes: `git pull`
- [ ] Deploy: `./packages/api.regarde.bio/deploy/scripts/deploy-production.sh`
...
ls -la packages/api.regarde.bio/deploy/scripts/

# Test script syntax
bash -n packages/api.regarde.bio/deploy/scripts/deploy-production.sh

# Run deployment with verbose output
bash -x packages/api.regarde.bio/deploy/scripts/deploy-production.sh
```
...
cp packages/api.regarde.bio/.env ~/api-backups/manual-$TIMESTAMP/
...
./packages/api.regarde.bio/deploy/scripts/manage-backups.sh clean

# Check backup disk usage
./packages/api.regarde.bio/deploy/scripts/manage-backups.sh size
