...
#
# Requirements:
# - .env file in packages/auth.regarde.dev/ with configuration
...
# Change to auth.regarde.dev directory
cd "$PROJECT_ROOT"
...
load_config() {
    ...
    if [ ! -f packages/auth.regarde.dev/.env ]; then
        log_error ".env file not found in packages/auth.regarde.dev/ directory!"
        log_info "Copy the template: cp packages/auth.regarde.dev/deploy/config/.env.template packages/auth.regarde.dev/.env"
        log_info "Then edit packages/auth.regarde.dev/.env with your actual values"
        exit 1
    fi

    # Load environment variables
    set -a
    source packages/auth.regarde.dev/.env
    set +a
