...
chmod +x packages/auth.regarde.dev/deploy/scripts/bootstrap-vm.sh
./packages/auth.regarde.dev/deploy/scripts/bootstrap-vm.sh

# 3. Configure environment
cp packages/auth.regarde.dev/deploy/config/.env.template packages/auth.regarde.dev/.env
nano packages/auth.regarde.dev/.env  # Edit with your actual values

# 4. Deploy the application
./packages/auth.regarde.dev/deploy/scripts/deploy-production.sh
```
...
All configuration is centralized in the `.env` file located in `packages/auth.regarde.dev/.env`:

> **Note**: The .env file is located in the auth.regarde.dev directory (not the project root) so that Bun can automatically load it at runtime using its native .env support.

```bash
# View current configuration
cat packages/auth.regarde.dev/.env

# Edit configuration
nano packages/auth.regarde.dev/.env

# Template with all options
cat packages/auth.regarde.dev/deploy/config/.env.template
```
...
# List backups
./packages/auth.regarde.dev/deploy/scripts/manage-backups.sh list

# Rollback to previous version
./packages/auth.regarde.dev/deploy/scripts/rollback.sh
```

## 📁 Project Structure

```
packages/auth.regarde.dev/deploy/
├── scripts/
...
- [ ] **Repository Setup**
  ...
  - [ ] Run bootstrap: `./packages/auth.regarde.dev/deploy/scripts/bootstrap-vm.sh`

- [ ] **Configuration**
  - [ ] Copy template: `cp packages/auth.regarde.dev/deploy/config/.env.template packages/auth.regarde.dev/.env`
  - [ ] Edit configuration: `nano packages/auth.regarde.dev/.env`
  - [ ] Set required values: `APP_PUBLIC_HOSTNAME`, `SSL_CERTIFICATE_EMAIL`, Jazz credentials

- [ ] **Deployment**
  - [ ] Run deployment: `./packages/auth.regarde.dev/deploy/scripts/deploy-production.sh`
  - [ ] Verify API: `curl -s https://your-domain.com/health | jq .`

### Regular Updates

- [ ] Pull changes: `git pull`
- [ ] Deploy: `./packages/auth.regarde.dev/deploy/scripts/deploy-production.sh`
...
ls -la packages/auth.regarde.dev/deploy/scripts/

# Test script syntax
bash -n packages/auth.regarde.dev/deploy/scripts/deploy-production.sh

# Run deployment with verbose output
bash -x packages/auth.regarde.dev/deploy/scripts/deploy-production.sh
```
...
cp packages/auth.regarde.dev/.env ~/api-backups/manual-$TIMESTAMP/
...
./packages/auth.regarde.dev/deploy/scripts/manage-backups.sh clean

# Check backup disk usage
./packages/auth.regarde.dev/deploy/scripts/manage-backups.sh size
