...
- **OnboardingAccount** schema (user's complete account):
...
- **JazzAppProfile** schema (user's public profile):
...
- **Namespace structure**:
  - `account.root["regarde.bio"]` = pre-loaded JazzAppProfile (use this)
  - `account.profile["regarde.bio"]` = string ID reference to JazzAppProfile CoMap
  - `account.root["auth.regarde.bio"]` = private RegistrationKey CoMap (24-hour expiry)
- **Registration key authentication (acts as 2FA)**:
  - User generates key via `useRegistrationKey()` (stored in `account.root["auth.regarde.bio"]`)
  - User sends key + key CoMap ID in headers (`X-Registration-Key`, `X-Registration-Key-Id`)
  - Worker loads RegistrationKey CoMap and verifies user owns it
...
- **Worker account loading**:
  ...
  - Worker loads account: `OnboardingAccount.load(accountId, { resolve: ... })`
...
- **Variable naming**: `account` for OnboardingAccount, `jazzAppProfile` for JazzAppProfile, `worker` for RegistryWorkerAccount, `registrationKey` for RegistrationKey